...
pub fn run() {
    ...
    let result_part_1 = calculate_part_1(&games);
    println!("Day 2, part 1: {}", result_part_1);

    let result_part_2 = calculate_part_2(&games);
    println!("Day 2, part 2: {}", result_part_2);
}
...
fn calculate_part_2(games: &Vec<Game>) -> u32 {
    games
        .iter()
        .map(|game| {
            let mut red: u32 = 0;
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green";
    ...
    let input: Vec<Game> = transform_input(input_text.split("\n").map(String::from).collect());
    assert_eq!(calculate_part_2(&input), 2286);
}
